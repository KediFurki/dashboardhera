...
<title>OperatorAvailableGlobal</title>
...
<table class="center"  >
	<tr>
		...
		<td>
			...
			<form id="form_operatoravailableglobalglobal" name="form_operatoravailableglobalglobal" action="OperatorAvailableGlobal.jsp" method="post">
...
<%
		...
		while (rs.next()) {
			...
			String transactionname = "";
 			...
 			String sFLAG_ATTIVA_OP_DISP = "--";
			
			log.info(session.getId() + " - dashboard.OperatorAvailable_GetAnnex('" + COD_IVR + "')");
			...
			while (rsCnf.next()) {
				...
				case "FLAG_ATTIVA_OP_DISP":
					...
					sFLAG_ATTIVA_OP_DISP = ((FLAG_ATTIVA_OP_DISP)?"ON":"OFF");
					break;
				}
			}
							...
							<td colspan="8">
								<table class="roundedCorners small no_top">
									<tr style="width: 100%" class="listGroupItem" >
										...
										<td><label class='switch'><input type='checkbox' name='FLAG_ATTIVA_OP_DISP'><span class='slider round green'></span></label></td>
									</tr>
								</table>
							</td>
						</tr>
...
%>
							...
							<td><label>(Flag=<%=sFLAG_ATTIVA_OP_DISP%></label> </td>
<%
				if (i%2==0) {
%>
						</tr>
...
<script type="text/javascript">
	...
	$(function() {
		try {
			parent.ChangeActiveMenu("#OperatorAvailableGlobal");
		} catch (e) {
		}
	})

...
<table class="center">
	<tr>
		...
		<td colspan="1">
			<form action="PerturbedTraffic.jsp" method="post">
<%
	...
	//		NONE
	//==============================
	boolean Perturbedtraffic = false;
	...
	try {
		if (StringUtils.isBlank(CodIvr)) {
			...
			if (PreviousState != null && CurrentState != null	&& !PreviousState.equalsIgnoreCase(CurrentState)) {
				log.info(session.getId() + " - dashboard.PerturbedTraffic_Update('"+environment+"','" + CodIvr + "','" + CurrentState+ "')");
				cstmt = conn.prepareCall("{ call dashboard.PerturbedTraffic_Update(?,?,?)} ");
				cstmt.setString(1,environment);
				cstmt.setString(2, CodIvr);
				cstmt.setString(3, CurrentState);
			} else {
				log.info(session.getId() + " - dashboard.PerturbedTraffic_Get('"+environment+"','" + CodIvr + "')");
					...
					<tr class="listGroupItem active blue">
						<td colspan="7">
							<table>
								<tr>
									<td style="width: 30%">
										<div class="title left" >Traffico Perturbato</div>
									</td>
									<td>
...
%>
				<div class="head_parameter_alert" style="text-align: center;">TRAFFICO PERTURBATO NON CONFIGURATO</div>
<%
			}
		}
	} catch (Exception e) {
...
<script type="text/javascript">
	$(function() {
		try {
			...
			parent.ChangeActivedFooter("#PerturbedTraffic","PerturbedTraffic.jsp");
		} catch (e) {}
		try {
<%	if (Perturbedtraffic) {%>

...
<title>PerturbedTrafficGlobal</title>
...
// 	boolean enableMod = true;
	...
	try {
		...
		switch (action) {
			case "delete" :
				log.info(session.getId() + " - dashboard.PerturbedTraffic_Update('"+environment+"','" + CodIvr + "','OFF'");
				cstmt = conn.prepareCall("{ call dashboard.PerturbedTraffic_Update(?,?,?)} ");
				...
				break;
			case "add" :
			    log.info(session.getId() + " - dashboard.PerturbedTraffic_Update('"+environment+"','" + CodIvr + "','ON')");
				cstmt = conn.prepareCall("{ call dashboard.PerturbedTraffic_Update(?,?,?)} ");
				cstmt.setString(1,environment);
				cstmt.setString(2, CodIvr);
				...
				break;
			case "deleteall" :
				log.info(session.getId() + " - dashboard.PerturbedTraffic_UpdateAll('"+environment+"','OFF')");
				cstmt = conn.prepareCall("{ call dashboard.PerturbedTraffic_UpdateAll(?,?)} ");
				cstmt.setString(1,environment);
				...
				break;
			case "addall" :
			    log.info(session.getId() + " - dashboard.PerturbedTraffic_UpdateAll('"+environment+"','ON')");
				cstmt = conn.prepareCall("{ call dashboard.PerturbedTraffic_UpdateAll(?,?)} ");
				cstmt.setString(1,environment);
	...
	//==	ACTION STOP		====
	try {
		log.info(session.getId() + " - dashboard.PerturbedTraffic_Get('"+environment+"',)");
		cstmt = conn.prepareCall("{ call dashboard.PerturbedTraffic_Get(?)} ");
		cstmt.setString(1,environment);
...
<table class="center"  >
	<tr>
		...
		<td>
				...
				<tr class="listGroupItem active green">
					<td colspan='7'>
						<table>
							<tr>
								<td>
									<div class="title">Traffico Perturbato</div>
								</td>
								<td valign="top">
			...
			<form id="form_save_perturbertrafficglobal" name="form_save_perturbertrafficglobal" action="PerturbedTrafficGlobal.jsp" method="post">
...
<script type="text/javascript">
	...
	$(function() {
		try {
			parent.ChangeActiveMenu("#PerturbedTrafficGlobal");
		} catch (e) {
		}
	})

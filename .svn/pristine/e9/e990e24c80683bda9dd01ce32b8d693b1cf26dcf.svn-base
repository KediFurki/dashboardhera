	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "upload":
				...
				String upload_location = cs.getProperty("file-special-location");
				upload_location += File.separator + username;
				File _Dir_upload = new File(upload_location);
				if (!_Dir_upload.exists()) {
					_Dir_upload.mkdirs();
				}  
				log.info(session.getId() + " - username : " + username + " - file-special-location : " + upload_location);
...
//				 		log.info(session.getId() + " - dashboard.AmbienteSpecial_DeleteAll('"+environment+"')");
//						cstmt = conn.prepareCall("{ call dashboard.AmbienteSpecial_DeleteAll(?)} ");
...
//						log.debug(session.getId() + " - records("+(x+1)+"):"+records.get(x));
//						String cRecord = records.get(x);
//						String[] record = cRecord.split(";");
//				 		log.info(session.getId() + " - dashboard.AmbienteSpecial_Add('"+environment+"','"+record[0]+"',"+((record.length>1)?"'"+record[1]+"'":"null")+")");
...
//					log.info(session.getId() + " - DB : Elaborato in "+totalTime+" ms");					
					...
					log.info(session.getId() + " - DB : inizio inserimeto record");
					pstmt = conn.prepareStatement("DELETE FROM ivr_configurazione.AmbienteSpecial" +
					...
					pstmt = conn.prepareStatement("INSERT INTO ivr_configurazione.AmbienteSpecial (" +
												  	"NUMERO_TELEFONO,DESCRIZIONE,TIME_STAMP,SERVIZIO" +
												  ") VALUES	(" +
												  	"?,?,?,'"+environment+"'" +
												  ")");
					for (Map.Entry<String,String> entry : mapRecords.entrySet()) {
						...
						pstmt.setTimestamp(3, new Timestamp(cal.getTime().getTime()));
						pstmt.addBatch();
					}

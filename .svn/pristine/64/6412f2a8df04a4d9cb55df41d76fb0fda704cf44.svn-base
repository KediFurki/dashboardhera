...
<table class="center"  >
	<tr>
		<td style="width: 5%"></td>
		<td style="width: 30%">
			<form id="form_recall_1" action="DistributionSTC.jsp" method="post">
...
<%
			...
			log.info(session.getId() + " - dashboard.Distribution_GetTransactions('" + CodIvr + "')");
			cstmt = conn.prepareCall("{ call dashboard.Distribution_GetTransactions(?)} ");
...
%>
				<tr class='listGroupItem' onclick='clickMe(event,this,"<%=transactionname%>") '>
					<td><div><%=transactionname%></div></td>
				</tr>
<%
			}
%>
		...
		<td style="width: 60%">
			<iframe src="DistributionSTCDetail.jsp" id="iFrame_DistributionSTCDetail" name="iFrame_DistributionSTCDetail" style="width: 100%; border: 0 solid grey"></iframe>
		</td>
		<td style="width: 5%"></td>
	</tr>
</table>
...
<div id="dates" class="modal">
	<form id="form_dates" name="form_dates" action="DistributionSTCDetail.jsp" method="post" target="iFrame_DistributionSTCDetail" >
		<input type="text" id="dst_transactionname" name="transactionname" > 
	</form>
</div>
...
<script>
	$(function() {
		try {
			...
			parent.ChangeActivedFooter("#DistributionSTC","DistributionSTC.jsp");
			$("#iFrame_DistributionSTCDetail").height($(window).height()-10 );

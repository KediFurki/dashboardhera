	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "upload":
				...
				String upload_location = cs.getProperty("file-top-location");
				upload_location += File.separator + username;
				File _Dir_upload = new File(upload_location);
				if (!_Dir_upload.exists()) {
					_Dir_upload.mkdirs();
				}  
				log.info(session.getId() + " - username : " + username + " - file-top-location : " + upload_location);
					...
					try {
						...
						if (deleteall.equalsIgnoreCase("ON")) {
							log.info(session.getId() + " - DB : inizio cancellazione");
					 		log.info(session.getId() + " - dashboard.Top_DeleteAll('"+environment+"')");
							cstmt = conn.prepareCall("{ call dashboard.Top_DeleteAll(?)} ");
							...
							pstmt = conn.prepareStatement("DELETE FROM ivr_configurazione.clienti_top" +
						...
						pstmt = conn.prepareStatement("INSERT INTO ivr_configurazione.clienti_top (" +

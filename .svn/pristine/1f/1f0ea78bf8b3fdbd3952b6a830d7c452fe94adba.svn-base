	...
	try {
		...
		switch (action) {
			case "delete":
				...
				log.info(session.getId() + " - dashboard.Top_Delete('"+environment+"','"+d_codcli+"')");
				cstmt = conn.prepareCall("{ call dashboard.Top_Delete(?,?)} ");
			...
			case "add":
				...
				String a_ntel5 = request.getParameter("add_ntel5");
		 		log.info(session.getId() + " - dashboard.Top_Add('"+environment+"'"+
													 				",'"+a_codcli+"'"+							// COD_CLI
													 				...
													 				")");
				cstmt = conn.prepareCall("{ call dashboard.Top_Add(?,?,?,?,?,?,?,?)} ");
				cstmt.setString(1,environment);
...
<table class="center">
	<tr>
		...
		<td style="width: 80%">
			<form id="form_without_control" action="TopGlobal.jsp" method="post">
					...
					<tr class="listGroupItem active green">
						<td colspan='9'>
							<table>
								<tr>
									<td style="width: 45%">
										<div class="title">Clienti TOP</div>
									</td>
									<td>
...
<%
	int numRows = 0;
	try {
		log.info(session.getId() + " - dashboard.Top_GetNumRows('"+environment+"')");
		cstmt = conn.prepareCall("{ call dashboard.Top_GetNumRows(?)} ");
		cstmt.setString(1,environment);
...
<div id="delete" class="modal">
	<div class="modal-content">
		...
		<form id="form_delete" action="TopGlobal.jsp" method="post">
...
<div id="add" class="modal">
	<div class="modal-content">
		<div class="modal-header green">
			<table style="width: 100%">
				<tr>
					<td>	<h2>Aggiungi Cliente TOP</h2></td>
					<td>
		...
		<form id="form_add" action="TopGlobal.jsp" method="post">
...
<div id="LoadFile" class="modal">
	<div class="modal-content">
		<div class="modal-header green">
			<table style="width: 100%">
				<tr>
					<td>	<h2>Carica Clienti TOP</h2></td>
					<td>
		...
		<form id="form_load_file" enctype="multipart/form-data">
			...
			<table style="width: 100%">
				...
				<tr>
					...
					<td><label>Cancella Clienti TOP presenti</label></td>
				</tr>
				<tr>
					<td>
				...
				<tr>
					<td colspan="2">
						<button type="button" id="Carica" class="button green" onclick="caricaTop();">Carica</button>
					</td>						
				</tr>
			</table>
		</form>
...
<div id="Reload" class="modal">
	<form id="form_reload" action="TopGlobal.jsp" method="post">
	</form>
</div>
...
<script>
	$(function() {
		try {
			parent.ChangeActiveMenu("#TopGlobal");			
		} catch (e) {
		}
	...
	$(".pagination").paging(<%=numRows%>, { // make 1337 elements navigatable
		...
		onSelect: function(page) {
			...
			$("#wait_image").addClass("imagevisible");
			$.ajax({
				url : 'DownloadTop?action=paging&page='+page,
				type : 'POST',
				contents: "json",
				processData: false,  // tell jQuery not to process the data
				contentType: false,  // tell jQuery not to set contentType

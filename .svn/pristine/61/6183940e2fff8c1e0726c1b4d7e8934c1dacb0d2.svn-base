			...
			case "upload":
				...
				try {
					...
					for (Map.Entry<String,String> entry : mapRecords.entrySet()) {
						log.info(session.getId() + " - dashboard.OutboundList_InsertCallingList('"+entry.getKey()+"','"+entry.getValue()+"')");
						cstmt = conn.prepareCall("{ call dashboard.OutboundList_InsertCallingList(?,?)} ");

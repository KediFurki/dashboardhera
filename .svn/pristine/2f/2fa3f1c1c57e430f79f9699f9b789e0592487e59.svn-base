...
public class DownloadTcp extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			switch (action) {
			case "paging":
				log.info(session.getId() + " - Start Paging Tcp");
				...
				log.info(session.getId() + " - dashboard.Tcp_GetPages(100,"+page+",'"+environment+"')");
				cstmt = conn.prepareCall("{ call dashboard.Tcp_GetPages(?,?,?)} ");
				cstmt.setInt(1,100);

...
<table class="center">
	<tr>
		...
		<td colspan="1">
			<form action="SelfReading.jsp" method="post">
<%
	...
	try {
		if (StringUtils.isBlank(CodIvr)) {
			...
			if (PreviousState != null && CurrentState != null	&& !PreviousState.equalsIgnoreCase(CurrentState)) {
				log.info(session.getId() + " - dashboard.SelfReading_Update('"+environment+"','" + CodIvr + "','" + CurrentState+ "')");
				...
				log.info(session.getId() + " - dashboard.SelfReading_Get('"+environment+"','" + CodIvr + "')");
					...
					<tr class="listGroupItem active blue">
						<td colspan="7">
							<table>
								<tr>
									<td style="width: 30%">
										<div class="title left" >AutoLettura (Alto Traffico)</div>
									</td>
									<td>
...
%>
				<div class="head_parameter_alert" style="text-align: center;">AUTOLETTURA (ALTO TRAFFICO) NON CONFIGURATO</div>
<%
			}
		}
	} catch (Exception e) {
...
<script type="text/javascript">
	$(function() {
		try {
			...
			parent.ChangeActivedFooter("#SelfReading","SelfReading.jsp");
		} catch (e) {}
		try {
<%	if (SelfReading) {%>

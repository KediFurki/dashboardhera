					    ...
					    for (int ija=0; ija<ja.length();ija++) {
							...
							while(jajokeys.hasNext()) {
								...
								if (jkey.equals("PRIORITA")) {
		 							log.info(session.getId() + " - key:"+jkey+" - value:"+jvalue+" -> "+priority);
									jajo.put(jkey, priority);
								}
							}
					    }
					}
...
<table class="center"  >
	<tr>
		...
		<td colspan="2" >
			<form id="form_emergency_1" action="ServiceGlobalCCTE.jsp" method="post">
...
<%
	try {
		String firstPage = "";
		log.info(session.getId() + " - dashboard.Service_ListEx('CCT-E')");
		cstmt = conn.prepareCall("{ call dashboard.Service_ListEx(?)}");
		cstmt.setString(1,"CCT-E");
		...
		while (rs.next()) {
			...
			firstPage = "'SetEmergency.jsp?CodIvr=" + COD_IVR + "'";
...
<%
			boolean Priority_Present = false;
			log.info(session.getId() + " - dashboard.Priority_GetAnnex('" + COD_IVR + "')");
			cstmtCnf = connCnf.prepareCall("{ call dashboard.Priority_GetAnnex(?)} ");
			cstmtCnf.setInt(1, Integer.parseInt(COD_IVR));
			rsCnf = cstmtCnf.executeQuery();					
			log.debug(session.getId() + " - executeCall complete");
			if (rsCnf.next()) {
				int cDbid =  rsCnf.getInt("dbid"); 
				String cKey = rsCnf.getString("key");
				String cValue = rsCnf.getString("value");
				int iPriorita = -1;
				try {
					iPriorita = Integer.parseInt(cValue);
				} catch (Exception e) {}
				String cTransactionname = rsCnf.getString("transactionname");
				if (iPriorita != -1) {
%>
						<td>
							<select id="Priorita" name="Priorita" onchange='changePriority(this,"<%=COD_IVR%>","<%=cDbid%>","<%=cTransactionname%>")'>
								<option value="1" <%=((iPriorita==1)?"selected":"") %>>1</option>
								<option value="2" <%=((iPriorita==2)?"selected":"") %>>2</option>
								<option value="3" <%=((iPriorita==3)?"selected":"") %>>3</option>
								<option value="4" <%=((iPriorita==4)?"selected":"") %>>4</option>
								<option value="5" <%=((iPriorita==5)?"selected":"") %>>5</option>
								<option value="6" <%=((iPriorita==6)?"selected":"") %>>6</option>
								<option value="7" <%=((iPriorita==7)?"selected":"") %>>7</option>
								<option value="8" <%=((iPriorita==8)?"selected":"") %>>8</option>
								<option value="9" <%=((iPriorita==9)?"selected":"") %>>9</option>
								<option value="10" <%=((iPriorita==10)?"selected":"") %>>10</option>
							</select>
						</td>
<%
...
<div id="Change_Survey" class="modal">
	<form id="form_change_survey" action="ServiceGlobalCCTE.jsp" method="post" >
...
<div id="Change_Priority" class="modal">
	<form id="form_change_priority" action="ServiceGlobalCCTE.jsp" method="post" >
		<input type="text" id="cpAction" name="action" Value="updPriority"> 
		<input type="text" id="cpCodIvr" name="codice" >
		<input type="text" id="cpDbid" name="dbid" >
		<input type="text" id="cpTransactionname" name="transactionname" >
...
<div id="Modify_Emergency" class="modal">
	<div class="modal-content">
		...
		<form id="form_modify_emergency" action="ServiceGlobalCCTE.jsp" method="post">
...
// 		$("#meTestoTts").css('height', scroll_height + 'px');
	...
	changePriority = function(_this,CodIvr,Dbid,Transactionname){
		$("#cpCodIvr").val(CodIvr);
		$("#cpDbid").val(Dbid);
		$("#cpTransactionname").val(Transactionname);

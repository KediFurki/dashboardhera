	...
	try {
		...
		switch (action) {
			case "delete":
				...
				log.info(session.getId() + " - dashboard.WhiteList_Delete('"+environment+"','"+d_ntel+"')");
				cstmt = conn.prepareCall("{ call dashboard.WhiteList_Delete(?,?)} ");
			...
			case "add":
				...
				log.info(session.getId() + " - dashboard.WhiteList_Add('"+environment+"','" + a_ntel + "','"+a_descrizione+"')");
...
<table class="center">
	<tr>
		...
		<td style="width: 80%">
			<form id="form_without_control" action="WhiteListGlobal.jsp" method="post">
					...
					<tr class="listGroupItem active green">
						<td colspan='7'>
							<table>
								<tr>
									<td style="width: 45%">
										<div class="title">White List</div>
									</td>
									<td>
...
<%
	try {
		log.info(session.getId() + " - dashboard.WhiteList_Get('"+environment+"')");
		cstmt = conn.prepareCall("{ call dashboard.WhiteList_Get(?)} ");
		cstmt.setString(1,environment);
...
%>
				<tr>
					<td><label><%=numero_telefono%></label> </td>
					<td><label><%=descrizione%></label> </td>
					<td><label><%=ts%></label> </td>
					<td>
					   <div  class='container_img grey right'>
						   <img src='images/TrashGrey_.png' onclick='confirmdelete("<%=numero_telefono%>","<%=descrizione%>")'  >
					    </div>
					</td>
				</tr>
<%
...
<div id="delete" class="modal">
	<div class="modal-content">
		...
		<form id="form_delete" action="WhiteListGlobal.jsp" method="post">
...
<div id="add" class="modal">
	<div class="modal-content">
		...
		<form id="form_add" action="WhiteListGlobal.jsp" method="post">
	...
	<div class="modal-content">
		...
		<form id="form_load_file" enctype="multipart/form-data">
			...
			<table style="width: 100%">
				...
				<tr>
					<td colspan="2">
						<button type="button" id="Carica" class="button green" onclick="caricaWhiteList();">Carica</button>
					</td>						
				</tr>
			</table>
		</form>
...
<div id="Reload" class="modal">
	<form id="form_reload" action="WhiteListGlobal.jsp" method="post">
	</form>
</div>
...
<script>
	$(function() {
		try {
			parent.ChangeActiveMenu("#WhiteListGlobal");			
		} catch (e) {
		}

...
@ServerEndpoint(value = "/WSSDetails", configurator = ServletAwareConfig.class)
public class WSSDetails implements IListener {
	Properties cs = null;
...
//				<td colspan='1' class='' id="ditails_calls"></td>
//				<td colspan='1' class='' id="ditails_entered"></td>
//				<td colspan='1' class='' id="ditails_hungupivr"></td>
//	 			<td colspan='1' class='' id="ditails_abbandonedivr"></td>
//				<td colspan='1' class='' id="ditails_exitivr"></td>
//	  			<td colspan='1' class='' id="ditails_dissuase"></td>
				ArrayList<JSONObject> al = DBUtility.getFlagStatisticsIvr(Environment, cod_ivr,stats);
				...
				///bug per questo tipo di report non è possibile effettuare calcoli ma il flags deve essere uguale alla stataistica
				JSONArray ja = DBUtility.getStatisticsIvr(Environment, false, cod_ivr,stats);
				...
				ja = DBUtility.getStatisticsIvr(Environment, true, cod_ivr,stats);
				log.info(session.getId() + " - Statistic: " + ja);
				for (int i = 0; i < ja.length(); i++) {
					JSONObject jo = ja.getJSONObject(i);
					hStat.put(jo.getString("ReportName")+"#"+jo.getString("Cod_IVR"), jo);
	...
	public static JSONObject recalc(Logger log, Hashtable<String, JSONObject> hStat, JSONObject op) throws JSONException, Exception {
		...
		for (; i >= 0; i--) {
			...
			try {
				...
				log.debug(" - error on threshold eval (" + ev + "): " + e.getMessage());
			}
		}
